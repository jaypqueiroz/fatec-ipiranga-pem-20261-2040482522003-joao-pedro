...
int ganhouDiagPrin(char s) {
    if (tab[0][0] == s && tab[1][1] == s && tab[2][2] == s){
        return 1;
    } 
    return 0;
}
...
int ganhouDiagSec(char s) {
    if (tab[0][2] == s && tab[1][1] == s && tab[2][0] == s) {
        return 1;
    }
    return 0;
}
...
// Pega os dados do jogador e ja tira 1 para a matriz
void pegarJogada(char player) {
    int linUser, colUser;
    printf("Vez do %c, escolha linha e coluna: ", player);
    scanf("%d%d", &linUser, &colUser);
    
    // O while garante que o cara nao jogue onde ja tem marca
    while (posicaoVazia(linUser - 1, colUser - 1) == 0) {
        printf("Lugar ocupado ou invalido! Tenta outro: ");
        scanf("%d%d", &linUser, &colUser);
    }
